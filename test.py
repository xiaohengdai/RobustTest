...
def set_per_key_value(data_json):
    # 获取包含各个子节点的列表
    check_json_value(data_json, 'id', '13333333333')
    ...
    for j in range(0, node_num_unrepeat):
        dic_json_deepcoy = check_json_value(data_json, node_list_num_unrepeat[j], 'displayName','xiaoheng')
        # print("data_json:",data_json)
        print("dic_json_deepcoy:", dic_json_deepcoy)

@on_response()
def change_response(flow):
    ...
    mock_flow_response=copy.deepcopy(flow_response)
    check_json_value(mock_flow_response, 'displayName', None)  #None可将其置为空
